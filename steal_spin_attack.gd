...
func steal_spin(spinner_one: RigidBody2D, spinner_two: RigidBody2D, damage: int):
	...
	if spinner_one_force > spinner_two_force:
		...
		spinner_two.health.takeDamage(spinner_two_damage)
		if spinner_two.health.CurrentHP < 0:
			spinner_one.health.heal(ceili(spinner_two_damage/2) + takedown_reward)
		else:
			spinner_one.health.heal(ceili(spinner_two_damage/2))
		...
		#print("Opponent Current HP: ", spinner_two.health.CurrentHP)
